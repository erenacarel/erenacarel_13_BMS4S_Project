...
bool UsrLcdInit(void)
{
    ...
    if(!lcdSendCommandProc(_USR_LCD_FUNCTION_SET | _USR_LCD_FUNCTION_SET_DATA_LENGTH)) // Function set -> DB5=1 and DB3=N=1  2-line display mode.      2x16 kullanılabilir
    	return false;
	if(!lcdSendCommandProc(_USR_LCD_DISPLAY_ON | _USR_LCD_CURSOR_ON | _USR_LCD_BLINK_ON))   // Display ON/OFF Control DB3=1 and DB2=D=1 display is turned on  // if DB1=C=1 cursor on
		return false;
